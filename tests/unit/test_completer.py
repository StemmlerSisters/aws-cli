...
class BaseCompleterTest(unittest.TestCase):
    ...
    def assert_completion(
        self, completer, cmdline, expected_results, point=None
    ):
        if point is None:
            point = len(cmdline)
...
class TestCompleter(BaseCompleterTest):
    def test_complete_services(self):
        commands = {
            'subcommands': {'foo': {}, 'bar': {'subcommands': {'baz': {}}}},
            'arguments': [],
        }
        completer = Completer(
    ...
    def test_complete_partial_service_name(self):
        ...
        self.assert_completion(
            completer, 'aws cloudf', ['cloudfront', 'cloudformation']
        )
    ...
    def test_complete_top_level_args(self):
        commands = {'subcommands': {}, 'arguments': ['foo', 'bar']}
        completer = Completer(
            self.clidriver_creator.create_clidriver(commands)
        )
        self.assert_completion(completer, 'aws --', ['--foo', '--bar'])
    ...
    def test_complete_partial_top_level_arg(self):
        ...
        self.assert_completion(
            completer, 'aws --f', ['--foo', '--fubar', '--foobar']
        )
        self.assert_completion(completer, 'aws --fo', ['--foo', '--foobar'])
    ...
    def test_complete_top_level_arg_with_arg_already_used(self):
        commands = {'subcommands': {'baz': {}}, 'arguments': ['foo', 'bar']}
        completer = Completer(
            self.clidriver_creator.create_clidriver(commands)
        )
    ...
    def test_complete_service_commands(self):
        commands = {
            'subcommands': {
                'foo': {
                    'subcommands': {'bar': {'arguments': ['bin']}, 'baz': {}}
                }
            },
            'arguments': [],
        }
    ...
    def test_complete_service_arguments(self):
        commands = {'subcommands': {'foo': {}}, 'arguments': ['baz', 'bin']}
        completer = Completer(
            self.clidriver_creator.create_clidriver(commands)
        )
    ...
    def test_complete_partial_service_arguments(self):
        ...
        self.assert_completion(
            completer, 'aws biz --f', ['--foo', '--fubar', '--foobar']
        )
        self.assert_completion(
            completer, 'aws biz --fo', ['--foo', '--foobar']
        )
    ...
    def test_complete_operation_arguments(self):
        commands = {
            'subcommands': {
                'foo': {'subcommands': {'bar': {'arguments': ['baz']}}}
            },
            'arguments': ['bin'],
        }
        completer = Completer(
    ...
    def test_complete_partial_operation_arguments(self):
        ...
        self.assert_completion(
            completer, 'aws foo bar --b', ['--base', '--baz', '--bin']
        )
        self.assert_completion(
            completer, 'aws foo bar --ba', ['--base', '--baz']
        )
    ...
    def test_complete_positional_argument(self):
        ...
        self.assert_completion(
            completer, 'aws foo bar --bin blah --', ['--baz']
        )
    ...
    def test_complete_undocumented_command(self):
        ...
        commands = {
            'subcommands': {'foo': {}, 'bar': UndocumentedCommand()},
            'arguments': [],
        }
        completer = Completer(
            self.clidriver_creator.create_clidriver(commands)
...
class TestCompleteCustomCommands(BaseCompleterTest):
    ...
    def test_complete_custom_command_arguments(self):
        self.assert_completion(
            self.completer, 'aws s3 cp --', ['--bar', '--recursive', '--sse']
        )

    def test_complete_custom_command_arguments_with_arg_already_used(self):
        self.assert_completion(
            self.completer, 'aws s3 cp --recursive --', ['--bar', '--sse']
        )


class MockCLIDriverFactory:
    ...
    def _create_operation_command(self, name, command):
        ...
        operation = ServiceOperation(
            name, 'parent', None, mock_operation, None
        )
